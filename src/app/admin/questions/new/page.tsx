...
export default function AdminQuestionNewPage() {
  ...
  const [form, setForm] = useState({
    task: 'TASK2', subtype: '', topic: '', content: '',
  ...
  async function handleSave() {
    setError('')
    if (!form.content.trim()) { setError('题目内容不能为空'); return }
    setSaving(true)
    try {
      ...
      if (form.outline) fd.append('outline', form.outline)
  ...
  return (
    <div style={outerStyle}>
      ...
      <button onClick={() => router.push('/admin/questions')} style={{
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#1e3a5f', marginBottom: 4 }}>新增题目</h1>
      ...
      <div style={{
        ...
        boxShadow: '0 2px 8px rgba(59,130,246,0.06)', padding: '28px', marginBottom: 16,
      }}>
        ...
        <div style={{ marginBottom: form.task === 'TASK1' ? 20 : 0 }}>
        ...
        {form.task === 'TASK1' && (
          <div>
            <div style={{ fontSize: 14, color: '#64748b', marginBottom: 10, fontWeight: 500 }}>
              题目图片（可选）
            </div>
            {imagePreview && (
              <div style={{ marginBottom: 14 }}>
                ...
                <button onClick={() => { setImageFile(null); setImagePreview(null) }}
                  style={{ display: 'block', marginTop: 8, fontSize: 13, color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                  × 移除图片
                </button>
              </div>
            )}
            <label style={{
              ...
              <span style={{ fontSize: 12, color: '#94a3b8' }}>支持 JPG / PNG</span>
      ...
      <div style={{
        background: '#fff', borderRadius: 14, border: '1.5px solid #e2e8f0',
        boxShadow: '0 2px 8px rgba(59,130,246,0.06)', padding: '28px',
      }}>
